...
interface ITurkeyGeoFeature {
  ...
  geometry: unknown; // GeoJSON geometry (kept broad to avoid extra deps)
}
...
export const CustomTurkeyMap: React.FC<CustomTurkeyMapProps> = ({
  ...
  const plateToName = useMemo(() => {
    const map = new Map<string, string>();
    for (const feature of turkeyGeoJson.features as ITurkeyGeoFeature[]) {
      const plate = String(feature.properties?.number ?? feature.properties?.plate ?? '').padStart(2, '0');
      const name = feature.properties?.name;
      if (plate && name) map.set(plate, name);
    }
    return map;
  }, []);
  ...
  const updateTooltipPosition = useCallback((clientX: number, clientY: number) => {
    if (tooltipRef.current && mapContainerRef.current) {
      ...
      tooltipRef.current.style.top = `${clientY - rect.top}px`;
    }
  }, []);
        ...
        {(turkeyGeoJson.features as ITurkeyGeoFeature[]).sort((a, b) => {
          ...
          return (
            <path
              ...
              onTouchStart={(e) => {
                setHoveredCity(plate);
                if (e.touches.length > 0) updateTooltipPosition(e.touches[0].clientX, e.touches[0].clientY);
              }}
              onMouseLeave={() => setHoveredCity(null)}
      ...
      {showTooltip && hoveredCity && (
        ...
        >
          <div className="bg-white/90 backdrop-blur-2xl border border-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] px-6 py-4 rounded-3xl min-w-[160px] flex flex-col gap-1 items-center justify-center animate-in zoom-in-95 fade-in duration-200">
            <span className="text-xs font-black text-slate-400 uppercase tracking-widest">{plateToName.get(hoveredCity) || 'Bilinmiyor'}</span>
