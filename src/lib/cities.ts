export const CITY_TO_PLATE: Record<string, string> = {
...
// Normalize city names for robust matching
export const normalizeCityName = (name: string): string => {
  return name.trim().replace('İ', 'I').replace('ı', 'i').toUpperCase();
};

export const getPlate = (cityName: string): string | undefined => {
  // Try direct match first
  if (CITY_TO_PLATE[cityName]) return CITY_TO_PLATE[cityName];
  
  // Try case-insensitive specific matches
  const normalized = normalizeCityName(cityName);
  for (const [key, val] of Object.entries(CITY_TO_PLATE)) {
